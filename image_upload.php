...
if (!isset($_REQUEST['ref'])) {
    ...
    $get_lastid = $conn->prepare("select * from image_details order by id desc");
    $get_lastid->execute();
...
if (isset($_REQUEST['savedata']) && $_REQUEST['savedata'] == 'savedata' && isset($_REQUEST['ref'])) {
    ...
    $title=$_POST['title'];
    $category_name=$_POST['category_name_details'];
    ...
    $query = "UPDATE `image_details` SET      title='$title',
...
if($imagepresent <= 0){
           ...
           $ext = pathinfo($filename, PATHINFO_EXTENSION);
                 if($ext == "jpeg" || $ext == "jpg" ||  $ext == "gif" || $ext == "png" || $ext == "webp"){
                            if(file_exists("images/portfolio/".$_FILES["image_name"]["name"])){
                                $FileERR = "file is already exit";
                            }else{
                                move_uploaded_file($_FILES["image_name"]["tmp_name"], "images/portfolio/".$_FILES["image_name"]["name"]);
                                $resume = $filename;
                            } 
...
if (isset($_REQUEST['savedata']) && $_REQUEST['savedata'] == 'savedata' && !isset($_REQUEST['ref'])) {
    ...
    $get_lastserial_value = $conn->prepare("select * from image_details order by serial desc " );
...
$insert_query =" insert into image_details (category_name, title, image_name, date_created, image_medium, image_size, serial, status) 
values ('$category_name','$title', '$filename',now(),'$image_medium', '$image_size','$last_serial_value','$status');";
...
$stmt = $conn->prepare("SELECT * FROM image_details where id='" . $ref . "'");
$stmt->execute();
...
<form id="form" enctype="multipart/form-data" name="form" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" >
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1 >
                <span style="font-size: 15px;"><a href="image_upload_list.php"> Home </a> > <a href="image_upload_list.php">Painting Upload</a>  > Edit </span>
            </h1>

            <ol class="breadcrumb">
                <li> 
                    <button style="margin-left:20px;margin-right: 30px;margin-top:-10px;" class="btn btn-info btn-icon" name="Back" id="back" type="button" title="Back" onclick="location.href = 'image_upload_list.php'" ><i class="fa fa-reply"></i></button>  
                            ...
                            <div class="tab-content">
                                <div class="active tab-pane" id="pagedata">
                                    ...
                                    <div class="form-group clearfix">
                                            ...
                                            <label class="col-md-2 control-label">Painting's Title</label>
                                            <div class="col-md-3">
                                            ...
                                            <div class="col-md-3">
                                                <!-- <input type="text" name="image_size"  class="form-control" value="<?php echo $arr['image_size']; ?>">  -->
                                                <select  name="category_name_details"  class="form-control"  >
                                                <option value="">Select Status</option>
                                                
                                                <?php 
                                                $arr_c = $conn->prepare("SELECT * FROM category where status='p' ");
                                                $arr_c->execute();



                                                while($arr_d = $arr_c->fetch(PDO::FETCH_ASSOC))
                                                {
                                                    ?>                                   
                                                
                                                
                                                <option value="<?php echo($arr_d['category_name']);?>"><?php echo($arr_d['category_name']);?></option>
                                                <?php } ?>
                                    ...
                                    <br/>
                                    <div class="form-group clearfix">
                                        <label class="col-md-2 control-label " >Painting Image</label>
                                        <div class="col-md-3">
                                            <input class="form-control" name="image_name" id="parallax"   type="file">
                                        </div>
                                        <div class= "display-other-images">
                                            <img id="blah" <?php if($arr['image_name'] !=null){ ?> src="images/portfolio/<?php echo $arr['image_name']; ?>"<?php } ?>alt="" width="260px;" />
                                        </div>   
                                    </div>
                                    <br/>
                                    <div class="form-group clearfix">

                                        <label class="col-md-2 control-label ">Painting Description</label>
                                        <div class="col-md-3">
                                            <textarea  name="image_medium" rows="4" cols="30"><?php echo $arr['image_medium']; ?></textarea>
                                        </div>

                                        <label class="col-md-2 control-label">Painting Size</label>
                                        <div class="col-md-3">
                                            <input type="text" name="image_size"  class="form-control" value="<?php echo $arr['image_size']; ?>"> 
                                        </div>
                                    </div>
