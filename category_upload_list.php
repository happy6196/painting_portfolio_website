...
if(isset($_REQUEST['action']) && $_REQUEST['action']!=""  ) // after form has been submitted
{
   ...
   if(isset($_REQUEST['ids']))
   {
		...
		{
		    $query=" UPDATE category SET status='p' WHERE id IN ($ids) ";
		...
		{
			$query=" UPDATE category SET status='u' WHERE id IN ($ids) ";
...
<head>
  ...
  <title>Painting's Category List</title>
  ...
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
     <section class="content-header">
        <h1 >
            <span style="font-size: 15px;"><a href=""> Home </a> > Painting's Category List</span>
      </h1>
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped" data-page-length='50'>
                ...
                ?>
                                    ...
                                    <td><input type="text" onchange="update_serialorder('<?php echo $arr['id']; ?>','category')"
    ...
    function updatestatus(elm,status) // fetching states 
{
  ...
  $.ajax({
    url: "ajax/update_status_blog_pages.php?id="+elm+"&status="+status+"&page=category",
